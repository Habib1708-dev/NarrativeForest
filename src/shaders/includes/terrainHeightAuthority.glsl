// Authority-Anchored Terrain Height Function
// Extended version of terrainHeight.glsl with freeflight coordinate offset support
//
// This module produces bit-exact results with the CPU version when in AUTHORED mode.
// In FREEFLIGHT mode, coordinates are offset by the travel offset to create
// an infinite terrain illusion while preserving the authored area's spatial integrity.

#ifndef TERRAIN_HEIGHT_AUTHORITY_GLSL
#define TERRAIN_HEIGHT_AUTHORITY_GLSL
...
uniform float uFreeflight;      // 0.0 = AUTHORED mode, 1.0 = FREEFLIGHT mode
...
// Main height function with authority-anchor support
// CRITICAL: Uses (x, -z) to mirror Z like the rotated plane did
// In FREEFLIGHT mode, applies travel offset to create infinite terrain illusion
float terrainHeightAt(float xWorld, float zWorld) {
  // Apply travel offset in freeflight mode
  // In AUTHORED mode (uFreeflight = 0), offset is zero
  // In FREEFLIGHT mode (uFreeflight = 1), offset shifts sampling coordinates
